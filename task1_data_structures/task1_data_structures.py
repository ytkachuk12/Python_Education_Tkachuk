...
class Queue:
    """Queue
    3 -> 2 -> 1. Tail it's the node at 0 index."""
    def __init__(self, value: Any) -> None:
        self.head = Node(value)
        self.tail = Node(value)
        self.length = 1
...
class BinarySearchTree:
    ...
    def lookup(self, value: Any) -> TreeNode | None:
        """Take value, set start node at root.
    ...
    def _lookup(self, hash_value: int, node: TreeNode) -> TreeNode | None:
...
class HashTable:
    ...
    def delete(self, key: Any) -> True:
        ...
        raise ValueError("ValueError: hash_table.delete(x): x not in tree")
